      ...
      <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm no-print">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-800">Laporan Rekap Logistik</h2>
          </div>
          ...
          Object.keys(groupedData).map((key) => {
            ...
            return (
              <div key={key} className="mb-8">
                ...
                <table className="w-full text-[9px] border-x border-b border-slate-200">
                  <thead>
                    <tr className="bg-slate-50 border-b border-slate-200">
                      <th className="px-4 py-1 text-left w-10">NO</th>
                      {tab !== 'vendor' && <th className="px-4 py-1 text-left w-24">TANGGAL</th>}
                      {tab !== 'vendor' && <th className="px-4 py-1 text-left">VENDOR</th>}
                      <th className="px-4 py-1 text-left">NAMA BARANG</th>
                      <th className="px-4 py-1 text-center w-16">QTY</th>
                      <th className="px-4 py-1 text-right w-28">SUBTOTAL</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-slate-100">
                    {rows.map((t: any, idx: number) => (
                      <tr key={idx}>
                        <td className="px-4 py-1 text-slate-400 font-bold">{idx + 1}</td>
                        {tab !== 'vendor' && <td className="px-4 py-1 font-medium">{t.Tanggal}</td>}
                        {tab !== 'vendor' && (
                          <td className="px-4 py-1 font-bold uppercase italic text-slate-600">{t.Vendor || '-'}</td>
                        )}
                        <td className="px-4 py-1 font-black uppercase">{t.NamaBarang}</td>
                        <td className="px-4 py-1 text-center font-black">{t.Qty}</td>
                        <td className="px-4 py-1 text-right font-black">
                  ...
                  <tfoot>
                    <tr className="bg-slate-50">
                      <td colSpan={tab === 'vendor' ? 3 : 5} className="px-4 py-1 text-right font-black text-slate-500 uppercase italic">
                        Sub Total
                      </td>
                      <td className="px-4 py-1 text-right font-black text-slate-900 border-t border-slate-200">
        ...
        <div className="mt-10 pt-6 border-t-2 border-slate-900 signature-block">
          ...
          <div className="flex justify-end mr-10">
            <div className="text-center">
              <p className="text-[8px] font-black text-slate-400 uppercase mb-12">Admin Logistik,</p>
