      ...
      <div id="po-print-area" className="bg-white p-10 mx-auto shadow-xl rounded-lg print:shadow-none print:border-none" style={{ width: '210mm', minHeight: '297mm', display: 'flex', flexDirection: 'column' }}>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-slate-900 text-white">
                <th className="px-4 py-1 text-[10px] font-black uppercase text-left w-12">No</th>
                <th className="px-4 py-1 text-[10px] font-black uppercase text-left">Nama Item</th>
                <th className="px-4 py-1 text-[10px] font-black uppercase text-center w-20">Qty</th>
                <th className="px-4 py-1 text-[10px] font-black uppercase text-right w-36">Total</th>
              </tr>
            </thead>
            <tbody className="border-x border-slate-200 divide-y divide-slate-100">
              {poItems.length === 0 ? (
                <tr>
                  <td colSpan={4} className="px-4 py-10 text-center text-slate-300 font-black text-[10px] uppercase">
                ...
                poItems.map((item, idx) => (
                  <tr key={item.NamaBarang || idx}>
                    <td className="px-4 py-1 text-[11px] font-bold text-slate-400">{idx + 1}</td>
                    <td className="px-4 py-1 text-[11px] font-black text-slate-900 uppercase">{item.NamaBarang}</td>
                    <td className="px-4 py-1 text-center text-[11px] font-black text-slate-900">{item.Qty}</td>
                    <td className="px-4 py-1 text-right text-[11px] font-black text-slate-900">
        ...
        <div className="mt-10 pt-6 border-t-4 border-slate-900 signature-block">
          <div className="flex justify-end items-center gap-6 mb-8">
            <p className="text-[11px] font-black text-slate-400 uppercase italic tracking-widest">TOTAL PO</p>
          ...
          <div className="flex justify-center gap-48 mb-10">
            <div className="text-center">
              <p className="text-[8px] font-black text-slate-400 uppercase mb-10">Dibuat Oleh,</p>
              <div className="w-32 border-b border-slate-900 mx-auto"></div>
              <p className="text-[8px] font-black text-slate-900 mt-1 uppercase">Admin Pengadaan</p>
            </div>
            <div className="text-center">
              <p className="text-[8px] font-black text-slate-400 uppercase mb-10">Disetujui Oleh,</p>
              <div className="w-32 border-b border-slate-900 mx-auto"></div>
              <p className="text-[8px] font-black text-slate-900 mt-1 uppercase">Manajer</p>
            </div>
          </div>
        </div>
        ...
        <div className="mt-auto pt-4 border-t border-slate-100 text-[8px] font-bold text-slate-300 uppercase tracking-[0.3em] print-footer-timestamp">
          e-Log - Logistic System RSDI Kendal - {new Date().toLocaleString('id-ID', { dateStyle: 'medium', timeStyle: 'short' })}
        </div>
      </div>
    </div>
  );
